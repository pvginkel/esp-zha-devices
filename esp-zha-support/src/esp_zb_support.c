...
esp_err_t esp_zb_endpoint_set_identity(esp_zb_cluster_list_t *ep, const char *manufacturer_name,
    ...
    auto err = esp_zb_basic_cluster_add_string_attr(basic_cluster, ESP_ZB_ZCL_ATTR_BASIC_MANUFACTURER_NAME_ID,
                                                    manufacturer_name);
    if (err != ESP_OK) {
        return err;
    }
...
esp_err_t esp_zb_basic_cluster_add_string_attr(esp_zb_attribute_list_t *attr_list, uint16_t attr_id,
    ...
    const auto err = esp_zb_basic_cluster_add_attr(attr_list, attr_id, copy);
