...
bool Debouncer::update() {
    ...
    if (readState != getStateFlag(DEBOUNCED_STATE)) {
        ...
        if (millis() - previous_millis >= interval_millis) {
            ...
            // This will be prompt as long as there has been greater than interval_misllis ms since last change of
            // input. Otherwise debounced state will not change again until bouncing is stable for the timeout period.
            changeState();
        }
    }
...
bool Debouncer::rose() const { return getStateFlag(DEBOUNCED_STATE) && getStateFlag(CHANGED_STATE); }

bool Debouncer::fell() const { return !getStateFlag(DEBOUNCED_STATE) && getStateFlag(CHANGED_STATE); }
...
void Bounce::attach(int pin, gpio_mode_t mode) {
    setPinMode(pin, mode);
    this->attach(pin);
}
